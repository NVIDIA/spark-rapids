...
class WindowedBlockIteratorSuite extends RapidsShuffleTestHelper {
  ...
  test ("1-byte+ ranges are allowed, but 0-byte or negative ranges are not") {
    assertResult(1)(new BlockRange(null, 123, 124).rangeSize())
    assertResult(2)(new BlockRange(null, 123, 125).rangeSize())
    assertThrows[IllegalArgumentException](new BlockRange(null, 123, 123))
    assertThrows[IllegalArgumentException](new BlockRange(null, 123, 122))
  }
