...
class UrlFunctionsSuite extends SparkQueryCompareTestSuite {
  ...
  def failedCases(session: SparkSession): DataFrame = {
    import session.sqlContext.implicits._
    Seq[String](
      "http://userinfo@spark.apache.org/path?query=1#Ref",
      "https://use%20r:pas%20s@example.com/dir%20/pa%20th.HTML?query=x%20y&q2=2#Ref%20two",
  ...
  def parseUrlHost(frame: DataFrame): DataFrame = {
    frame.selectExpr("urls", "parse_url(urls, 'HOST')")
  }
  ...
  testSparkResultsAreEqual("Test parse_url failed cases", failedCases) {
    parseUrlHost
  }
}