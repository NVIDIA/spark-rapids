...
class GpuPostProcessorSuite extends AnyFunSuite with BeforeAndAfterAll {
  ...
  // --- isCompatibleForCoalescing tests ---

  private def makeProcessor(
      parquetSchema: ShadedMessageType,
      expectedSchema: Schema,
      idToConstant: JHashMap[Integer, Any] = new JHashMap[Integer, Any](),
      filePath: String = "/test/file.parquet"
  ): GpuParquetReaderPostProcessor = {
    val block = createBlockMetaData(10)
    val info = ParquetFileInfoWithBlockMeta(
    ...
    new GpuParquetReaderPostProcessor(info, idToConstant, expectedSchema, parquetSchema, Map.empty)
  }

  test("isCompatibleForCoalescing: identical schemas and constants are compatible") {
    val col1Id = 1
    val col2Id = 2
    val colType1 = ShadedTypes.primitive(ShadedPrimitiveTypeName.INT64, ShadedRepetition.OPTIONAL)
      .id(col1Id).named("a")
    val colType2 = ShadedTypes.primitive(ShadedPrimitiveTypeName.DOUBLE, ShadedRepetition.OPTIONAL)
      .id(col2Id).named("b")
    val parquetSchema = new ShadedMessageType("test", Seq(colType1, colType2).asJava)
    val expectedSchema = new Schema(
      Types.NestedField.optional(col1Id, "a", Types.LongType.get()),
      Types.NestedField.optional(col2Id, "b", Types.DoubleType.get())
    )

    val proc1 = makeProcessor(parquetSchema, expectedSchema, filePath = "/test/file1.parquet")
    val proc2 = makeProcessor(parquetSchema, expectedSchema, filePath = "/test/file2.parquet")

    assert(proc1.isCompatibleForCoalescing(proc2))
    assert(proc2.isCompatibleForCoalescing(proc1))
  }

  test("isCompatibleForCoalescing: different file schemas are incompatible") {
    val col1Id = 1
    val col2Id = 2
    val col3Id = 3
    ...
    val colType2 = ShadedTypes.primitive(ShadedPrimitiveTypeName.DOUBLE, ShadedRepetition.OPTIONAL)
      .id(col2Id).named("b")
    val colType3 = ShadedTypes.primitive(ShadedPrimitiveTypeName.INT32, ShadedRepetition.OPTIONAL)
      .id(col3Id).named("c")

    val schema1 = new ShadedMessageType("test", Seq(colType1, colType2).asJava)
    val schema2 = new ShadedMessageType("test", Seq(colType1, colType3).asJava)

    val expectedSchema = new Schema(
      Types.NestedField.optional(col1Id, "a", Types.LongType.get()),
      Types.NestedField.optional(col2Id, "b", Types.DoubleType.get()),
      Types.NestedField.optional(col3Id, "c", Types.LongType.get())
    )

    val proc1 = makeProcessor(schema1, expectedSchema)
    val proc2 = makeProcessor(schema2, expectedSchema)

    assert(!proc1.isCompatibleForCoalescing(proc2))
  }

  test("isCompatibleForCoalescing: same schemas but different constants are incompatible") {
    val col1Id = 1
    val constColId = 2
    val colType = ShadedTypes.primitive(ShadedPrimitiveTypeName.INT64, ShadedRepetition.OPTIONAL)
      .id(col1Id).named("a")
    val parquetSchema = new ShadedMessageType("test", Seq[ShadedType](colType).asJava)
    val expectedSchema = new Schema(
      Types.NestedField.optional(col1Id, "a", Types.LongType.get()),
      Types.NestedField.optional(constColId, "partition_col", Types.LongType.get())
    )

    val const1 = new JHashMap[Integer, Any]()
    const1.put(constColId, 42L)
    val const2 = new JHashMap[Integer, Any]()
    const2.put(constColId, 99L)

    val proc1 = makeProcessor(parquetSchema, expectedSchema, const1)
    val proc2 = makeProcessor(parquetSchema, expectedSchema, const2)

    assert(!proc1.isCompatibleForCoalescing(proc2))
  }

  test("isCompatibleForCoalescing: FetchFilePath makes processors incompatible") {
    val col1Id = 1
    val filePathId = MetadataColumns.FILE_PATH.fieldId()
    val colType = ShadedTypes.primitive(ShadedPrimitiveTypeName.INT64, ShadedRepetition.OPTIONAL)
      .id(col1Id).named("a")
  ...
  test("isCompatibleForCoalescing: FetchRowPosition makes processors incompatible") {
    val col1Id = 1
    val rowPosId = MetadataColumns.ROW_POSITION.fieldId()
    val colType = ShadedTypes.primitive(ShadedPrimitiveTypeName.INT64, ShadedRepetition.OPTIONAL)
      .id(col1Id).named("a")
    ...
    val proc1 = makeProcessor(parquetSchema, expectedSchema)
    val proc2 = makeProcessor(parquetSchema, expectedSchema)
