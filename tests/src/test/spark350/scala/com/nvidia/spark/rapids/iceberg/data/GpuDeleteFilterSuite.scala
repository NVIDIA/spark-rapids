...
import com.nvidia.spark.rapids.iceberg.{fieldIndex, toSparkType, PooledTableGen}
import com.nvidia.spark.rapids.iceberg.data.TestGpuDeleteLoader._
import com.nvidia.spark.rapids.iceberg.parquet.{GpuIcebergParquetReaderConf, GpuParquetReaderConf, SingleFile}
...
class GpuDeleteFilterSuite extends AnyFunSuite with BeforeAndAfterAll {
  ...
  private def fixture(deleteFiles: Seq[DeleteFile], _tableSchema: Schema = TABLE_SCHEMA) = new {
    ...
    val eqFieldIdSets = deleteFiles
      ...
      .map(_.equalityFieldIds().asScala.map(_.toInt))
      .distinct
  ...
  test("Filter with equality deletes only") {
    ...
    val testColumns = Table(("eqFieldId", "eqFieldName"),
      TABLE_SCHEMA.columns()
        ...
        .map { fields =>
          (fields.map(_.fieldId()), fields.map(_.name()))
        }.toSeq: _*)
...
//      NestedField.optional(8, "time", Types.TimeType.get()),
  ...
  def tableSchemaWithIsDeletedColumn(): Schema = {
    val allFields = TABLE_SCHEMA.columns().asScala :+ MetadataColumns.IS_DELETED
  ...
  def gpuDeleteFilterOf(
      ...
      deleteLoader: Option[GpuDeleteLoader]): GpuDeleteFilter2 = {
    new GpuDeleteFilter2(
      tableSchema,
      Map.empty,
      GpuIcebergParquetReaderConf(
        GpuParquetReaderConf(
          caseSensitive = false,
          new Configuration(),
          100,
          100000,
