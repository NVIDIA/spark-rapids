...
object GpuOverrides extends Logging {
  ...
  // the only shim changin is Spark 3.3.X so leave off for now
  ...
  val commonExpressions: Map[Class[_ <: Expression], ExprRule[_ <: Expression]] = Seq(
    ...
    expr[GetArrayItem](
      ...
      (in, conf, p, r) => new BinaryExprMeta[GetArrayItem](expr, conf, parent, rule)),
    expr[GetMapValue](
      ...
      (in, conf, p, r) => new BinaryExprMeta[GetMapValue](expr, conf, parent, rule)),
    expr[ElementAt](
