...
def test_array_transform(data_gen):
    def do_it(spark):
        columns = ['a',
                'transform(a, x -> x) as ident',
                'transform(a, x -> null) as n',
                'transform(a, x -> 1) as one']
        element_type = data_gen.data_type.elementType
        # decimal types can grow so we are avoiding those here for now
        if isinstance(element_type, IntegralType):
            columns.extend(['transform(a, x -> x + 1) as add',
                'transform(a, x -> x + x) as mul'])

        if isinstance(element_type, StringType):
            columns.extend(['transform(a, x -> concat(x, "-test")) as con'])

        if isinstance(element_type, ArrayType):
            columns.extend(['transform(a, x -> transform(x, y -> 1)) as sub_one'])
