...
def _assert_gpu_and_cpu_are_equal(func,
    ...
    conf={},
    is_cpu_first=True,
    result_converter_func_before_compare=None):
    ...
    (from_cpu, from_gpu) = (run_on_cpu(), run_on_gpu()) if is_cpu_first else (run_on_gpu(), run_on_cpu())
    print('### {}: GPU TOOK {} CPU TOOK {} ###'.format(collect_type,
        gpu_end - gpu_start, cpu_end - cpu_start))

    if result_converter_func_before_compare is not None:
...
def assert_gpu_and_cpu_are_equal_collect(func, conf={}, is_cpu_first=True, result_converter_func_before_compare=None):
    """
    Assert when running func on both the CPU and the GPU that the results are equal.
    In this case the data is collected back to the driver and compared here, so be
    careful about the amount of data returned.
    :param result_converter_func_before_compare: specify a func to convert the cpu and gpu results before compare.
                                                 ...
                                                 The result of fastparquet is different from the result of GPU, e.g.:
                                                  ...
                                                  Use this converter func to canonicalize the results.
                                                  Usage of converter func is: (cpu, gpu) = result_converter_func_before_compare(original_cpu_result, original_gpu_result)
    """
    _assert_gpu_and_cpu_are_equal(func, 'COLLECT', conf=conf, is_cpu_first=is_cpu_first, result_converter_func_before_compare=result_converter_func_before_compare)
