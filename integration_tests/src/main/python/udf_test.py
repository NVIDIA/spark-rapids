...
from spark_session import is_databricks104_or_later, is_before_spark_330
...
@pytest.mark.skipif(not is_databricks104_or_later(), reason="zero-conf is supported only from db9.1")
@ignore_order(local=True)
...
@pytest.mark.skipif(is_databricks104_or_later(), reason="This is tested by other tests from db9.1")
@ignore_order(local=True)
@pytest.mark.parametrize('data_gen', [LongGen()], ids=idfn)
