...
@pytest.mark.parametrize("reader_confs", reader_opt_confs, ids=idfn)
@pytest.mark.parametrize('end_timestamp', [datetime(2199, 1, 1, tzinfo=timezone.utc), None], ids=idfn)
@pytest.mark.parametrize('v1_enabled_list', ["", "orc"])
# ORC writer and reader timezones are both controlled by the JVM default timezone
# (TimeZone.getDefault()), not by spark.sql.session.timeZone. The JVM TZ is set
# by the TZ env var configured for the test run (see @tz_sensitive_test).
@tz_sensitive_test
def test_orc_non_utc_timezone(reader_confs, end_timestamp, spark_tmp_path, v1_enabled_list):
    ...
    with_cpu_session(lambda spark: gen_df(spark, date_timestamp_gens).write.orc(cpu_write_path), conf=confs)
    assert_gpu_and_cpu_are_equal_collect(read_orc_df(cpu_write_path), conf=confs)
