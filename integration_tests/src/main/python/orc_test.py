...
from asserts import *
from conftest import is_not_utc, is_Shanghai_timezone
...
non_utc_allow_orc_file_source_scan = ['ColumnarToRowExec', 'FileSourceScanExec', 'BatchScanExec'] \
    if is_not_utc() and (not is_Shanghai_timezone()) else []
...
_with_data_writing_and_orc_file_scan = non_utc_allow_orc_file_source_scan + ["DataWritingCommandExec", "WriteFilesExec"] \
    if is_not_utc() and (not is_Shanghai_timezone()) else []
@pytest.mark.parametrize("reader_confs", reader_opt_confs, ids=idfn)
@pytest.mark.parametrize('v1_enabled_list', ["", "orc"])
@pytest.mark.parametrize("timezone_pair", [("UTC", "Asia/Shanghai"), ("Asia/Shanghai", "UTC"), ("Asia/Shanghai", "America/Los_Angeles")], ids=idfn)
@allow_non_gpu(*_with_data_writing_and_orc_file_scan)
@tz_sensitive_test
def test_orc_non_utc_timezone(reader_confs, spark_tmp_path, v1_enabled_list, timezone_pair):
    ...
    ts_gen = TimestampGen(start=datetime(1970, 1, 1, tzinfo=timezone.utc), nullable=True)
    ...
    with_gpu_session(lambda spark: gen_df(spark, date_timestamp_gens).write.orc(gpu_write_path), conf=write_confs)
    assert_gpu_and_cpu_are_equal_collect(read_orc_df(gpu_write_path), conf=read_confs)

@pytest.mark.skipif(True, reason='https://github.com/NVIDIA/spark-rapids/issues/13272: CPU can not read ORC file generated by GPU when timestamp is less than 1970 year')
@allow_non_gpu(*non_utc_allow_orc_file_source_scan)
