...
all_basic_no_nan_gens = [byte_gen, short_gen, int_gen, long_gen, FloatGen(no_nans=True), DoubleGen(no_nans=True),
        string_gen, boolean_gen, date_gen, timestamp_gen]

@pytest.mark.parametrize('data_gen', all_basic_no_nan_gens, ids=idfn)
def test_least(data_gen):
    ...
    assert_gpu_and_cpu_are_equal_collect(
            lambda spark : gen_df(spark, gen).select(
                f.least(*command_args)), conf={'spark.rapids.sql.hasNans': 'false'})
...
def test_greatest(data_gen):
    ...
    assert_gpu_and_cpu_are_equal_collect(
            lambda spark : gen_df(spark, gen).select(
                f.greatest(*command_args)), conf={'spark.rapids.sql.hasNans': 'false'})
