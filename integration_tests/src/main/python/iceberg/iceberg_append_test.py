...
def test_insert_into_unpartitioned_table(spark_tmp_table_factory, format_version):
...
def test_insert_into_unpartitioned_table_values(spark_tmp_table_factory, format_version,
                                                partition_table):
    base_table_name = get_full_table_name(spark_tmp_table_factory)
    cpu_table_name = f"{base_table_name}_cpu"
    gpu_table_name = f"{base_table_name}_gpu"
...
@pytest.mark.parametrize("format_version", ["1", "2"], ids=lambda x: f"format_version={x}")
def test_insert_into_unpartitioned_table_all_cols_fallback(spark_tmp_table_factory, format_version):
    table_prop = {"format-version": format_version}
...
def test_insert_into_partitioned_table(spark_tmp_table_factory, format_version, partition_col_sql):
    table_prop = {"format-version": format_version}
...
@allow_non_gpu('AppendDataExec', 'ShuffleExchangeExec', 'ProjectExec')
@pytest.mark.parametrize("format_version", ["1", "2"], ids=lambda x: f"format_version={x}")
def test_insert_into_partitioned_table_all_cols_fallback(spark_tmp_table_factory, format_version):
    table_prop = {"format-version": format_version}
