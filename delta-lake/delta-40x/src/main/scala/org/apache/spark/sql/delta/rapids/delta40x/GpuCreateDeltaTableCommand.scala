...
case class GpuCreateDeltaTableCommand(
      ...
      val result = query match {
        ...
        case Some(query) =>
          ...
          val data = org.apache.spark.sql.rapids.shims.TrampolineConnectShims
            .createDataFrame(TrampolineConnectShims.getActiveSession, query)
  ...
  private def handleCreateTable(
    ...
    def createActionsForNewTableOrVerify(): Seq[Action] = {
      ...
      if (noExistingMetadata && !allowRestoringExistingEmptySchemaTable) {
        ...
        val activeSpark: SparkSession = TrampolineConnectShims.getActiveSession
  ...
  private def replaceMetadataIfNecessary(
    ...
    if (txn.readVersion > -1L && isReplace && !dontOverwriteSchema) {
      ...
      val sparkSession: SparkSession = TrampolineConnectShims.getActiveSession
      val updatedConfig = UniversalFormat.enforceDependenciesInConfiguration(
          sparkSession,
          newMetadata.configuration,
          txn.snapshot)
