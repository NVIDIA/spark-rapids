...
case class GpuUpdateCommand(
  ...
  final override def run(sparkSession: SqlSparkSession): Seq[Row] = {
    recordDeltaOperation(tahoeFileIndex.deltaLog, "delta.dml.update") {
      ...
      gpuDeltaLog.withNewTransaction(catalogTable) { txn =>
        ...
        performUpdate(ClassicSparkSession.active, deltaLog, txn)
      }
      // Re-cache all cached plans(including this relation itself, if it's cached) that refer to
      // this data source relation.
      val classic = ClassicSparkSession.active
      classic.sharedState.cacheManager.recacheByPlan(classic, target)
    }
    Seq(Row(metrics("numUpdatedRows").value))
  }

  private def performUpdate(sparkSession: ClassicSparkSession, deltaLog: DeltaLog,
    ...
    val filesToRewrite: Seq[TouchedFileWithDV] = if (candidateFiles.isEmpty) {
      ...
      val touchedFilesWithDV = if (shouldWriteDeletionVectors) {
        ...
        val data = org.apache.spark.sql.rapids.shims.TrampolineConnectShims
          .createDataFrame(sparkSession, newTarget)
        ...
        val pathsToRewrite =
          withStatusCode("DELTA", UpdateCommand.FINDING_TOUCHED_FILES_MSG) {
            data.filter(org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(updateCondition))
  ...
  private def rewriteFiles(
    ...
    val (targetDf, finalOutput, finalUpdateExpressions) = UpdateCommand.preserveRowTrackingColumns(
      targetDfWithoutRowTrackingColumns = org.apache.spark.sql.rapids.shims.TrampolineConnectShims
        .createDataFrame(spark, newTarget),
      snapshot = txn.snapshot,
      targetOutput = target.output,
      updateExpressions)

    val targetDfWithEvaluatedCondition = {
      val evalDf = targetDf.withColumn(
        UpdateCommand.CONDITION_COLUMN_NAME,
        org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(condition))
      val copyAndUpdateRowsDf = if (copyUnmodifiedRows) {
        evalDf
      } else {
        evalDf.filter(org.apache.spark.sql.functions.col(UpdateCommand.CONDITION_COLUMN_NAME))
      }
  ...
  def shouldWritePersistentDeletionVectors(
      spark: ClassicSparkSession, txn: GpuOptimisticTransactionBase): Boolean = {
