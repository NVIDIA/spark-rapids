...
case class GpuDeleteCommand(
  ...
  final override def run(sparkSession: SqlSparkSession): Seq[Row] = {
    ...
    recordDeltaOperation(gpuDeltaLog.deltaLog, "delta.dml.delete") {
      gpuDeltaLog.withNewTransaction(catalogTable) { txn =>
        ...
        val (deleteActions, deleteMetrics) = performDelete(
          sparkSession.asInstanceOf[ClassicSparkSession], deltaLog, txn)
      ...
      case Some(cond) =>
        ...
        if (otherPredicates.isEmpty) {
          ...
          if (shouldWriteDVs) {
            ...
            val data = TrampolineConnectShims.createDataFrame(sparkSession, newTarget)
            ...
            val filesToRewrite =
              withStatusCode("DELTA", FINDING_TOUCHED_FILES_MSG) {
                ...
                data.filter(DFUDFShims.exprToColumn(cond))
            ...
            if (filesToRewrite.isEmpty) {
              ...
              val targetDF = RowTracking.preserveRowTrackingColumns(
                dfWithoutRowTrackingColumns = TrampolineConnectShims.createDataFrame(
                  sparkSession, newTarget), snapshot = txn.snapshot)
      ...
      val dfToWrite = if (shouldWriteCdc) {
        ...
        baseData
            ...
            .withColumn(
              ...
              DFUDFShims.exprToColumn(If(filterCondition, CDC_TYPE_NOT_CDC, CDC_TYPE_DELETE))
            )
      } else {
        baseData
            .filter(numTouchedRowsUdf())
            .filter(DFUDFShims.exprToColumn(filterCondition))
      }
  ...
  def shouldWritePersistentDeletionVectors(
      spark: ClassicSparkSession, txn: OptimisticTransaction): Boolean = {
