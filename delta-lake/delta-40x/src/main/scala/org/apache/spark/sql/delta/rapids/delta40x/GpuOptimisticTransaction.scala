...
class GpuOptimisticTransaction(deltaLog: DeltaLog,
  ...
  private def getGpuStatsColExpr(
    ...
    val analyzedExpr = org.apache.spark.sql.rapids.shims.TrampolineConnectShims
      .createDataFrame(TrampolineConnectShims.getActiveSession, LocalRelation(statsDataSchema))
    ...
    analyzedExpr.transform { case rr: RuntimeReplaceable => rr.replacement }
  }
  ...
  private def getOptionalGpuStatsTrackerAndStatsCollection(
    ...
    if (spark.sessionState.conf.getConf(DeltaSQLConf.DELTA_COLLECT_STATS)) {
      ...
      val statsCollection = new GpuStatisticsCollection {
        override protected def spark: SparkSession = TrampolineConnectShims.getActiveSession
  ...
  override def writeFiles(
    ...
    val gpuWritePlan = TrampolineConnectShims.createDataFrame(
      spark, RapidsDeltaWrite(normalizedQueryExecution.logical))
