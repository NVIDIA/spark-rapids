...
abstract class DeltaProviderBase extends DeltaIOProvider {
  ...
  override def canPushDVPredicateDownToScan(conf: SQLConf): Boolean = {
    ...
    // All three reader modes (PERFILE, MULTITHREADED, COALESCING) now support deletion vectors
    // via GpuDeltaParquetFileFormatBase2, so no per-mode check is needed here.
