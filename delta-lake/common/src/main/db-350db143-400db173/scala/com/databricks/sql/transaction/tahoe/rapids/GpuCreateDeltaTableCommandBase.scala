 ...
 * @param query The query to commit into the Delta table if it exists.
 */
abstract class GpuCreateDeltaTableCommandBase(
    ...
    @transient protected val rapidsConf: RapidsConf)
  ...
  protected def validateQueryPlan(plan: LogicalPlan): Unit = {}
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    recordDeltaOperation(gpuDeltaLog.deltaLog, "delta.ddl.createTable") {
      ...
      if (query.isDefined) {
        ...
        val queryPlan = query.get
        validateQueryPlan(queryPlan)
        // We are either appending/overwriting with saveAsTable, creating a new table with CTAS,
        // or creating a table as part of a RunnableCommand.
        queryPlan match {
          ...
          case other =>
            ...
            val data = TrampolineConnectShims.createDataFrame(
              sparkSession.asInstanceOf[TrampolineConnectShims.SparkSession], other)
