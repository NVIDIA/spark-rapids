...
import com.nvidia.spark.rapids.delta.GpuDeltaParquetFileFormatBase.{METADATA_ROW_IDX_COLUMN, METADATA_ROW_IDX_FIELD}
    ...
    private val targetDF = {
      ...
      val newPlan = {
        val rowIdxAttr = AttributeReference(
          GpuDeltaParquetFileFormatBase.METADATA_ROW_IDX_COLUMN ,
          GpuDeltaParquetFileFormatBase.METADATA_ROW_IDX_FIELD.dataType,
          GpuDeltaParquetFileFormatBase.METADATA_ROW_IDX_FIELD.nullable)()
        ...
        baseTargetDF.queryExecution.analyzed.transformUp {
          case r@LogicalRelation(fs: HadoopFsRelation, _, _, _) =>
            val newSchema = StructType(fs.dataSchema.fields).add(
              GpuDeltaParquetFileFormatBase.METADATA_ROW_IDX_FIELD)
