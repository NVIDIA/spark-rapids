...
object GpuLowShuffleMergeCommand {
  ...
  trait MergeExecutor extends Logging {
  ...
  class LowShuffleMergeExecutor(
    ...
    private def writeModified(processedDF: DataFrame): Seq[FileAction] = {
      val outputDF = processedDF
        .drop(ROW_DROPPED_COL, ROW_ID_COL, FILE_PATH_COL)
      logDebug("Calculating changes: join output plan:\n" + outputDF.queryExecution.analyzed)
    ...
    private def writeUnmodified(processedDF: DataFrame): Seq[FileAction] = {
      // Schema of processedDF: targetSchema + ROW_DROPPED_COL + ROW_ID_COL + FILE_PATH_COL
      // It consists of two pars:
      // 1. Unmatched source rows which are inserted
      // 2. Modified target rows which are updated
