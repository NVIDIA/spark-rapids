...
object GpuLowShuffleMergeCommand {
  ...
  /**
   * Used to identify a row in a dataframe, usually generated by [[monotonically_increasing_id]].
   * Unlike [[GpuDeltaParquetFileFormatUtils.METADATA_ROW_IDX_COL]], it has no requirement on
   * file scan. Currently it is only used in [[LowShuffleMergeExecutor.findTouchedFiles]] to
   * test duplicated rows.
   */
  val ROW_ID_COL: String = "_metadata_row_id"
  val FILE_PATH_COL = GpuDeltaParquetFileFormatUtils.FILE_PATH_COL
...
//        .localCheckpoint(true)
    ...
    private def updateOutputMetrics(processedDF: DataFrame): Unit = {
      val result = processedDF.selectExpr(
        s"""count_if($METADATA_ROW_IDX_COL IS NULL AND
           |!$ROW_DROPPED_COL) as insertedCount""".stripMargin,
        s"""count_if($ROW_DROPPED_COL AND $ROW_MATCHED_COL AND
           |$METADATA_ROW_IDX_COL IS NOT NULL) as deletedMatchedCount""".stripMargin,
        s"""count_if($ROW_DROPPED_COL AND !$ROW_MATCHED_COL AND
           |$METADATA_ROW_IDX_COL IS NOT NULL)
    ...
    private def writeModified(processedDF: DataFrame): Seq[FileAction] = {
      val outputDF = processedDF
        ...
        .drop(ROW_DROPPED_COL, ROW_MATCHED_COL, METADATA_ROW_IDX_COL, FILE_PATH_COL)
    ...
    private def writeUnmodified(processedDF: DataFrame): Seq[FileAction] = {
      ...
      val modifiedTargetRows = processedDF
        .filter(col(METADATA_ROW_IDX_COL).isNotNull) // Filter out unmatched source rows
        .select(
          (col(METADATA_ROW_IDX_COL) * -1).as(METADATA_ROW_IDX_COL),
          col(FILE_PATH_COL))
    ...
    : Seq[Seq[Expression]] = {
      val updateExprs = {
        val mainDataOutput = resolvedActions.map(_.expr) :+
          ...
          matched :+
          UnresolvedAttribute(METADATA_ROW_IDX_COL) :+
    ...
    private def makeUnmodifiedTargetDF(
      ...
      val joinCondition = baseTargetDF(FILE_PATH_COL) === modifiedTargetRows(FILE_PATH_COL) &&
        ((baseTargetDF(METADATA_ROW_IDX_COL) +
          modifiedTargetRows(METADATA_ROW_IDX_COL)) === 0L)


      baseTargetDF.join(modifiedTargetRows,
          joinCondition, "anti")
        .drop(METADATA_ROW_IDX_COL, FILE_PATH_COL)
    }
