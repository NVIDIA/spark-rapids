...
<a name="sql.castStringToTimestamp.enabled"></a>spark.rapids.sql.castStringToTimestamp.enabled|When set to false, disable casting from string to timestamp on the GPU. This is only for test purpose: some cases rely on casting string to timestamp is disabled.|true|Runtime
