...
<a name="sql.expression.cpuBridge.enabled"></a>spark.rapids.sql.expression.cpuBridge.enabled|Enable CPU-GPU bridge expressions that allow CPU expression subtrees to run while keeping the overall plan on GPU. When enabled, expressions that have no GPU implementation will automatically be wrapped in bridge expressions instead of causing plan fallbacks.|true|Runtime
