...
**_Note:_** Using the CUDA 450.80.02 driver in combination with the CUDA 10.1 or 10.2 toolkit may
result in long read times when reading a file that is snappy compressed.  If using the CUDA
450.80.02 driver we recommend running with the CUDA 11.0 toolkit.  This issue is resolved in the 0.5
and higher release.  
