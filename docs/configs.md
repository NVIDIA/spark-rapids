...
<a name="sql.fallbackDeltaCheckpoint"></a>spark.rapids.sql.fallbackDeltaCheckpoint|Fall back to the CPU when we see Delta Lake _delta_log Parquet checkpoint files being read. Many times we see the overhead of doing columnar conversions very high for these queries.|true|Runtime
