...
<a name="sql.opt.allTrueFalse.enabled"></a>spark.rapids.sql.opt.allTrueFalse.enabled|When set to true, checks whether the predictions on a batch are all true or false, then accordingly evaluate only the true or false expression. This is for the GPU `If` and `CaseWhen`, and can improve the performance by avoiding the evaluation of the true or false expression when it is an expensive operation. e.g. row-based UDF. It is disabled by default because it does work when the input batch is mixed with true and false predictions, and may affect the performance negatively because it involves some additional computations. This config might be removed in the future when the implementation of GPU `If` and `CaseWhen` evolves|false
