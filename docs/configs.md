...
<a name="sql.parseExtendedYears.enabled"></a>spark.rapids.sql.parseExtendedYears.enabled|SPARK-35780 went into Spark 3.2.0+ and extended parsing of years in dates and timestamps to support the full range of possible values. Prior to this it was limited to a positive 4 digit year. When this config is enabled we will parse dates and timestamps on the GPU, even though we do not support the extended range yet.|false
