...
<a name="sql.incompatibleDateFormats.enabled"></a>spark.rapids.sql.incompatibleDateFormats.enabled|When parsing strings as dates and timestamps in functions like unix_timestamp, some formats are fully supported on GPU, some are supported but can produce incorrect results for invalid inputs, and others are not supported at all. Setting this to true will force all parsing onto GPU for supported formats, including formats that can result in incorrect results.|false
