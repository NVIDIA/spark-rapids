...
<a name="sql.format.parquet.multithreaded.combine.threshold"></a>spark.rapids.sql.format.parquet.multithreaded.combine.threshold|When using the multithreaded parquet reader, if other files are already ready, combine them together up to this threshold size before sending down to GPU. This can be disabled by setting it to 0.|67108864|Runtime
