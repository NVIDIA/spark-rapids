...
<a name="sql.typedImperativeAggregate.enabled"></a>spark.rapids.sql.typedImperativeAggregate.enabled|Allow running TypedImperativeAggregate functions on the GPU. Note that TypedImperativeAggregate will fallback to run entirely on the CPU if part of the aggregate can't be accelerated.|false
