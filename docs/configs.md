...
-----|-------------|--------------
spark.rapids.memory.gpu.allocPct|The percentage of total GPU memory that should be initially allocated for pooled memory. Extra memory will be allocated as needed, but it may result in more fragmentation.|0.9
spark.rapids.memory.gpu.asyncSpillPct|When reported GPU memory usage is above this percentage, cached data will spill asynchronously.|0.75
...
spark.rapids.memory.gpu.spillPct|When reported GPU memory usage is above this percentage, the allocator thread will block until spilling of cached data has completed.|0.85
