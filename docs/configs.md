...
<a name="sql.outOfCoreSort.enabled"></a>spark.rapids.sql.outOfCoreSort.enabled|enable or disable out of core sorting. Out of core sorting allows for sorting large skewed data sets, but is not a stable sort. Apache Spark does not guarantee a stable sort in most cases, except when coalescing all data to a single task and sorting there. In that case we will produce different results.|true
