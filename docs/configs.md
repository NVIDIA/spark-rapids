...
<a name="spark.rapids.shuffle.ucx.listener.enabled"></a>spark.rapids.shuffle.ucx.listener.enabled|When set to true, start listener and exchange socket address. This allows better detecting remote peer failures.|false
<a name="spark.rapids.shuffle.ucx.peerErrorHandling.enabled"></a>spark.rapids.shuffle.ucx.peerErrorHandling.enabled|When set to true, enable peer error handling for ucx endpoints. Could impact on a transports and protocols, being selected in ucx.|false
