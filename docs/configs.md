...
<a name="sql.format.parquet.multithreaded.combine.sizeBytes"></a>spark.rapids.sql.format.parquet.multithreaded.combine.sizeBytes|When using the multithreaded parquet reader, if other files are already ready, combine them together up to this threshold size before sending down to GPU. This can be disabled by setting it to 0. Note that combine also will not go over the spark.rapids.sql.reader.batchSizeRows or spark.rapids.sql.reader.batchSizeBytes limits.|67108864|Runtime
