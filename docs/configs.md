...
<a name="sql.decimalType.enabled"></a>spark.rapids.sql.decimalType.enabled|Enable decimal types up to 64-bit precision. Note that not all GPU operations support decimal, so enabling them could have performance implications.|false
