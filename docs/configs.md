...
<a name="sql.cpuBasedUDF.enabled"></a>spark.rapids.sql.cpuBasedUDF.enabled|When set to true, supports to run a CPU based UDF efficiently by transferring only the data it needs between GPU and CPU inside a plan, instead of falling the whole plan back to CPU. This is an experimental feature, and this conf might be removed in the future.|false
