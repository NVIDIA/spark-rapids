...
<a name="memory.gpu.arena.enabled"></a>spark.rapids.memory.gpu.arena.enabled|Should RMM use the arena allocator for GPU memory, or should it use the default pooling allocator.|true
...
<a name="python.memory.gpu.arena.enabled"></a>spark.rapids.python.memory.gpu.arena.enabled|Should RMM in Python workers use the arena allocator for GPU memory, or should it use the default pooling allocator. When not specified, It will honor the value of config 'spark.rapids.memory.gpu.arena.enabled'|None
