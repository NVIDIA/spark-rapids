...
<a name="sql.window.range.decimal.enabled"></a>spark.rapids.sql.window.range.decimal.enabled|Support for DECIMAL in range based window functions is tentative. When set to false disables the range window acceleration for the DECIMAL type order-by column|true
