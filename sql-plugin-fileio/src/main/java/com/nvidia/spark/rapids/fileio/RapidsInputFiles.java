...
public final class RapidsInputFiles {
    ...
    private static final GCSPerfReader DISABLED_GCS_PERF_READER = new GCSPerfReader() {
        @Override
        public boolean isEnabled() {
            return false;
        }
        ...
        @Override
        public RapidsInputFile createInputFile(Path path, Configuration conf) {
            return null;
        }
    };

    private static volatile GCSPerfReader gcsPerfReader = DISABLED_GCS_PERF_READER;
    ...
    /**
     * Java bridge for GCS PerfIO integration. The implementation lives in sql-plugin
     * because it depends on private Scala PerfIO state.
     */
    public interface GCSPerfReader {
        boolean isEnabled();

        RapidsInputFile createInputFile(Path path, Configuration conf) throws IOException;
    }
    ...
    public static void setGCSPerfReader(GCSPerfReader reader) {
        gcsPerfReader = Objects.requireNonNull(reader, "reader can't be null");
    }

    public static void resetGCSPerfReader() {
        gcsPerfReader = DISABLED_GCS_PERF_READER;
    }
    ...
    /**
     * True iff the active SQL-plugin bridge says the GCS PerfIO path is enabled.
     * Returns false before the bridge is registered so callers default to the
     * non-PerfIO path during early bring-up.
     */
    public static boolean isGCSPerfEnabled() {
        return gcsPerfReader.isEnabled();
    }
    ...
    public static RapidsInputFile createGCSInputFile(Path path, Configuration conf)
            throws IOException {
        return gcsPerfReader.createInputFile(path, conf);
    }
}
