...
/** Hadoop input factory that routes cloud paths through the registered PerfIO bridges. */
public final class PerfIOHadoopInputFileFactory implements HadoopInputFileFactory {
    ...
    public RapidsInputFile create(Path path, Configuration conf) throws IOException {
        ...
        if (scheme != null && (scheme.equals("gs") || scheme.equals("gcs")) &&
            ...
            RapidsInputFile inputFile = RapidsInputFiles.createGCSInputFile(path, conf);
            if (inputFile != null) {
                return inputFile;
            }
        }
