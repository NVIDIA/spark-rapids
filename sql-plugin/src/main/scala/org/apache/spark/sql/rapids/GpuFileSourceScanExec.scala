...
import com.nvidia.spark.rapids.{GpuExec, GpuMetricNames, GpuParquetMultiFilePartitionReaderFactory, GpuReadCSVFileFormat, GpuReadFileFormatWithMetrics, GpuReadOrcFileFormat, GpuReadParquetFileFormat, SparkPlanMeta, RapidsConf}
...
case class GpuFileSourceScanExec(
  ...
  lazy val inputRDD: RDD[InternalRow] = {
    val useSmallFileOpt = canUseSmallFileOptimization
    val readFile: Option[(PartitionedFile) => Iterator[InternalRow]] = if (!useSmallFileOpt) {
    ...
    val readRDD = if (bucketedScan) {
      createBucketedReadRDD(relation.bucketSpec.get, readFile, dynamicallySelectedPartitions,
        relation, useSmallFileOpt)
    } else {
      createNonBucketedReadRDD(readFile, dynamicallySelectedPartitions, relation, useSmallFileOpt)
    }
    sendDriverMetrics()
    readRDD
  }
  ...
  private def createBucketedReadRDD(
    ...
    if (!useSmallFileOpt) {
      new FileScanRDD(fsRelation.sparkSession, readFile.get, filePartitions)
    } else {
  ...
  private def createNonBucketedReadRDD(
    ...
    if (!useSmallFileOpt) {
      new FileScanRDD(fsRelation.sparkSession, readFile.get, partitions)
    } else {
      // here we are making an optimization to read more then 1 file at a time on the CPU side
      // if they are small files before sending it down to the GPU
      val sqlConf = relation.sparkSession.sessionState.conf
  ...
  private def canUseSmallFileOptimization: Boolean = {
    // get up to date conf because isInputFileExecUsed is set at runtime
    val rapidsConf = new RapidsConf(relation.sparkSession.sessionState.conf)
    val formatSupportsSmallFilesOptimization = relation.fileFormat match {
