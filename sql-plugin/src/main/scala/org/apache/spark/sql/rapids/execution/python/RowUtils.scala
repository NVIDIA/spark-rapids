...
// The whole file is copied from Spark `RowQueue` to expose row queue utils to rapids execs
 ...
 */
private[sql] abstract class InMemoryRowQueue(val page: MemoryBlock, numFields: Int)
  extends RowQueue {
 ...
 */
private[sql] case class DiskRowQueue(
    file: File,
    fields: Int,
 ...
 * time.
 */
private[sql] case class HybridRowQueue(
