...
import com.nvidia.spark.rapids.shims.v2.{ShimBinaryExecNode, SparkShimImpl}
...
case class GpuFlatMapCoGroupsInPandasExec(
  ...
  override def requiredChildOrdering: Seq[Seq[SortOrder]] = {
    leftGroup.map(SparkShimImpl.sortOrder(_, Ascending)) ::
      rightGroup.map(SparkShimImpl.sortOrder(_, Ascending)) :: Nil
  }
