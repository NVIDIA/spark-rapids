...
import org.apache.spark.sql.rapids.{CudfAggregate, GpuAggregate, GpuAggregateExpression}
      ...
      try {
        ...
        try {
          if (aggregatedCb == null && groupingExpressions.isEmpty) {
            ...
            val defaultValues =
              aggregateFunctions.asInstanceOf[Seq[GpuAggregate]].flatMap(_.initialValues)
  ...
  def setupReferences(childAttr: AttributeSeq,
    ...
    val updateExpressionsSeq =
      aggregateExpressions.map(
        _.aggregateFunction.asInstanceOf[GpuAggregate].updateExpressions)
    //
    ...
    val mergeExpressionsSeq =
      aggregateExpressions.map(
        _.aggregateFunction.asInstanceOf[GpuAggregate].mergeExpressions)
    ...
    val mergeExpressionsNonDistinct =
      nonDistinctAggExpressions
        .flatMap(_.aggregateFunction.asInstanceOf[GpuAggregate].mergeExpressions)
    ...
    val finalProjections = groupingExpressions ++
      aggregateExpressions.flatMap(_.aggregateFunction.asInstanceOf[GpuAggregate]
        .evaluateExpression)
