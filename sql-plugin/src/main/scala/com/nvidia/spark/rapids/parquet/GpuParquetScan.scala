...
import org.apache.parquet.schema.{DecimalMetadata, GroupType, LogicalTypeAnnotation, MessageType, PrimitiveType, Type, Types}
...
trait ParquetPartitionReaderBase extends Logging with ScanWithMetrics
   ...
   */
  protected def buildSchemaFromBlocks(blocks: Seq[BlockMetaData], originalSchema: MessageType): MessageType = {
    if (blocks.isEmpty) {
