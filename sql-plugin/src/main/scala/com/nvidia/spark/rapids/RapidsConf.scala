...
object RapidsConf {
  ...
  def help(asTable: Boolean = false, commonlyUsed: Option[Boolean] = None): Unit = {
    // commonlyUsed is used to print commonly used configs or advanced configs, if it is None,
    // print all configs
    if (asTable) {
      println("---")
      println("layout: page")
      if (commonlyUsed.isEmpty || commonlyUsed.get == true) {
        ...
        println("""# RAPIDS Accelerator for Apache Spark Advanced Configuration
          |Most users will not need to modify the configuration options listed below. They are 
          |documented here for completeness and advanced usage.
          |
          |The following configuration options are supported by the RAPIDS Accelerator for Apache 
          |Spark.
          |
          |For commonly used configurations and examples of setting options, please refer to the 
          |[RAPIDS Accelerator for Configuration](https://nvidia.github.io/spark-rapids/docs/
          |configs.html) page.
          |""".stripMargin)
    ...
    val outputConfs = commonlyUsed match {
      case Some(true) => allConfs.filter(_.isCommonlyUsed)
      case Some(false) => allConfs.filter(!_.isCommonlyUsed)
      case None => allConfs
    }
    outputConfs.sortBy(_.key).foreach(_.help(asTable))
    if (commonlyUsed.get == true) {
      if (asTable) {
        println("""For more advanced configs, please refer to the [RAPIDS Accelerator for Apache 
          |Spark Advanced Configuration page](https://nvidia.github.io/spark-rapids/docs/
          |additional-functionality/advanced_configs.html).
          |""".stripMargin)
      }
      // Skip the Supported GPU Operators and Fine Tuning if we are printing commonly used configs
      return
    }
    if (asTable) {
      println("")
