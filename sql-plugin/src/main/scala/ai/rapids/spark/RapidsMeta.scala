...
abstract class RapidsMeta[INPUT <: BASE, BASE, OUTPUT <: BASE](
   ...
   */
  final def convertIfNeeded(): BASE =
   ...
   */
  protected def print(append: StringBuilder, depth: Int): Unit = {
    indent(append, depth)
    append.append(if (canThisBeReplaced) "*" else "!")
    ...
    append.append(willWorkOnGpuInfo)
      .append("\n")

    printChildren(append, depth)
  }
