...
case class GpuShuffleExchangeExec(
  ...
  extends Exchange with ShuffleExchangeExecLike with GpuExec {
  ...
  @transient lazy val mapOutputStatisticsFuture: Future[MapOutputStatistics] = {
    if (inputBatchRDD.getNumPartitions == 0) {
      ...
      sparkContext.submitMapStage(shuffleBatchDependencyColumnar)
    }
  }
   ...
   */
  @transient
  lazy val shuffleBatchDependencyColumnar : ShuffleDependency[Int, ColumnarBatch, ColumnarBatch] = {
