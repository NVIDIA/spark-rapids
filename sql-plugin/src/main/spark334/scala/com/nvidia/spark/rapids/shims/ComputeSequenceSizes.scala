...
import org.apache.spark.sql.rapids.{AddOverflowChecks, GpuSequenceUtil, SubtractOverflowChecks}
      ...
      withResource(Scalar.fromInt(MAX_ROUNDED_ARRAY_LENGTH)) { maxLen =>
        withResource(sizeAsLong.lessOrEqualTo(maxLen)) { allValid =>
          require(isAllValidTrue(allValid),
            s"Unsuccessful try to create array with elements exceeding the array size " +
              s"limit $MAX_ROUNDED_ARRAY_LENGTH")
        }
      }
