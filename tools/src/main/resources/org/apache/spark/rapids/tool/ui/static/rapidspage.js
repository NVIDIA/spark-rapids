...
function setRapidsHistoryAppLimit(val) {
    appLimit = val;
}
    ...
    $.getJSON(uiRoot + "/api/v1/applications", appParams, function(response, _ignored_status, _ignored_jqXHR) {
        ...
        for (var i in response) {
            ...
            for (var j in app["attempts"]) {
                ...
                attempt["action"] = (currentAppID == id ? 0 : 1)
                attempt["attemptUrl"] = uiRoot + "/history/" + id + "/" +
                    (attempt.hasOwnProperty("attemptId") ? attempt["attemptId"] + "/" : "") + "RAPIDS/";
                attempt["compareUrl"] = (currentAppID != id ? uiRoot + "/history/" + currentAppID + "/RAPIDS/compare/?id=" +
                    id : "");
                array.push(attempt);
            }
        ...
        $.get(uiRoot + "/static/rapids/rapids-historypage-template.html", function(template) {
            var sibling = historySummary.prev();
            historySummary.detach();
            var apps = $(Mustache.render($(template).filter("#rapids-history-summary-template").html(),data));
                ...
                "columns": [
                    {
                        ...
                        type: "appid-numeric",
                        data: 'id',
                        render:  (id, type, row) => (id != currentAppID ? `<span title="${id}"><a href="${row.attemptUrl}">${id}</a></span>` : `<span title="${id}">${id}</span>`)
                    },
            ...
            $('#rapids-history-summary [data-toggle="tooltip"]').tooltip();
        });
    });
});
