...
class ApplicationInfoSuite extends FunSuite with Logging {
  ...
  test("test filename match") {
    ...
    val (result, allEvents) = EventLogPathProcessor.processAllPaths(appArgs.filterCriteria.toOption,
  ...
  test("test filter file newest") {
    ...
    try {
      ...
      val (result, allLogs) = EventLogPathProcessor.processAllPaths(appArgs.filterCriteria.toOption,
        appArgs.matchEventLogs.toOption, appArgs.eventlog(),
        sparkSession.sparkContext.hadoopConfiguration)
      assert(result.length == 2)
