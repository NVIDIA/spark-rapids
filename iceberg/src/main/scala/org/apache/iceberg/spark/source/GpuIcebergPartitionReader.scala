...
class GpuIcebergPartitionReader(private val task: GpuSparkInputPartition,
  ...
  private lazy val table = task.cpuInputPartition.table()
  ...
  private def collectFiles() = {
    val tasks: Seq[FileScanTask] = task.cpuInputPartition.taskGroup()
  ...
  private def newConf(): GpuIcebergParquetReaderConf = {
    ...
    GpuIcebergParquetReaderConf(
      task.cpuInputPartition.isCaseSensitive,
      task.hadoopConf.value.value,
      task.rapidsConf.maxReadBatchSizeRows,
      task.rapidsConf.maxReadBatchSizeBytes,
      task.rapidsConf.gpuTargetBatchSizeBytes,
      task.rapidsConf.maxGpuColumnSizeBytes,
      task.rapidsConf.chunkedReaderEnabled,
      task.maxChunkedReaderMemoryUsageSizeBytes,
      task.rapidsConf.parquetDebugDumpPrefix,
      task.rapidsConf.parquetDebugDumpAlways,
      metrics,
      threadConf,
      task.expectedSchema,
      nameMapping)
