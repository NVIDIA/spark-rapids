...
class SparkSchemaConverter extends TypeWithSchemaVisitor[DataType] {
  ...
  override def struct(iStruct: Types.StructType, struct: GroupType, fieldTypes: JList[DataType])
    ...
    val fields: JArrayList[StructField] = Lists.newArrayListWithExpectedSize(fieldTypes.size)
    for (i <- 0 until parquetFields.size) {
      val parquetField = parquetFields.get(i)
      Preconditions.checkArgument(!parquetField.isRepetition(ParquetType.Repetition.REPEATED),
  ...
  override def list(iList: Types.ListType, array: GroupType, elementType: DataType): DataType = {
    ...
    Preconditions.checkArgument(!element.isRepetition(ParquetType.Repetition.REPEATED),
  ...
  DataType = {
    ...
    Preconditions.checkArgument(!value.isRepetition(ParquetType.Repetition.REPEATED),
