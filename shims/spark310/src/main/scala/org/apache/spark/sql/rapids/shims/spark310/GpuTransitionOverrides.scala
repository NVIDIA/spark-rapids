...
import com.nvidia.spark.rapids.{CoalesceGoal, GpuBringBackToHost, GpuCoalesceBatches, GpuExec, GpuExpression, GpuHashAggregateExec, GpuOverrides, GpuRowToColumnarExec, GpuSortExec, HostColumnarToGpu, RapidsConf, RequireSingleBatch, TargetSize}
import com.nvidia.spark.rapids.shims.spark301.GpuBroadcastHashJoinExec
...
import org.apache.spark.sql.execution.exchange.{BroadcastExchangeLike, Exchange, ShuffleExchangeExec}
...
class GpuTransitionOverrides extends Rule[SparkPlan] {
  ...
  def assertIsOnTheGpu(plan: SparkPlan, conf: RapidsConf): Unit = {
    ...
    plan match {
      case _: Exchange if isAdaptiveEnabled &&
