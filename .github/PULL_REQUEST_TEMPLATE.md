...
<!-- GPU & Compatibility Checklist (reference — check items if applicable):
- All GPU resources (ColumnarBatch, GpuColumnVector, etc.) use
  withResource/closeOnExcept — no bare .close().
- If shim files were modified, all related Spark version shims are
  updated consistently.
- If pom.xml was modified, ./build/make-scala-version-build-files.sh
  2.13 has been run to sync Scala 2.13 pom files.
- If new RapidsConf keys were added, they have documentation and
  sensible defaults.
- If new GPU operators were added, they are registered in
  GpuOverrides with proper fallback declarations.
- If this PR affects Databricks-specific code, [databricks] is in
  the PR title.
- If SNAPSHOT dependency versions were changed, the rationale is
  described above.
-->
...
If no, write 'No'.
Please refer to [CONTRIBUTING.md](https://github.com/NVIDIA/spark-rapids/blob/HEAD/CONTRIBUTING.md) for guidance on AI-assisted authorship.
-->
