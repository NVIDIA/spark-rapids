...
mvn_verify() {
    ...
    # Download a Scala 2.12 version of Spark
...
ci_scala213() {
    ...
    prepare_spark $SPARK_VER_213 2.13
    ...
    $MVN_CMD -U -B $MVN_URM_MIRROR clean package $MVN_BUILD_ARGS -DskipTests=true
    cd .. ## Run integration tests in the project root dir to levarage test cases and resources files
    export TEST_TAGS="not premerge_ci_1"
    export TEST_TYPE="pre-commit"
    export TEST_PARALLEL=5
