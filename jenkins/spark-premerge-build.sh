...
mvn_verify() {
    ...
    [[ $BUILD_MINOR_VERSION_SNAPSHOTS == "true" ]] && env -u SPARK_HOME mvn -U -B $MVN_URM_MIRROR -Dbuildver=314 clean install -Drat.skip=true -DskipTests -Dmaven.javadoc.skip=true -Dskip -Dmaven.scalastyle.skip=true -Dcuda.version=$CUDA_CLASSIFIER -pl aggregator -am
    ...
    [[ $BUILD_MINOR_VERSION_SNAPSHOTS == "true" ]] && env -u SPARK_HOME mvn -U -B $MVN_URM_MIRROR -Dbuildver=322 clean install -Drat.skip=true -DskipTests -Dmaven.javadoc.skip=true -Dskip -Dmaven.scalastyle.skip=true -Dcuda.version=$CUDA_CLASSIFIER -pl aggregator -am
    ...
    [[ $BUILD_MAJOR_VERSION_SNAPSHOTS == "true" ]] && env -u SPARK_HOME mvn -U -B $MVN_URM_MIRROR -Dbuildver=340 clean install -Drat.skip=true -DskipTests -Dmaven.javadoc.skip=true -Dskip -Dmaven.scalastyle.skip=true -Dcuda.version=$CUDA_CLASSIFIER -pl aggregator -am
...
BUILD_MINOR_VERSION_SNAPSHOTS="false"
# controls whether we build snapshots for the next Spark major version like 3.4.0 or 4.0.0
BUILD_MAJOR_VERSION_SNAPSHOTS="false"
...
if [[ ${PROJECT_VER} =~ ^22\.08\. ]]; then # enable snapshot builds for active development branch only
  BUILD_MINOR_VERSION_SNAPSHOTS="true"
  BUILD_MAJOR_VERSION_SNAPSHOTS="false"
  PREMERGE_PROFILES="-Psnapshots,pre-merge"
elif [[ ${PROJECT_VER} =~ ^22\.10\. ]]; then
  BUILD_MINOR_VERSION_SNAPSHOTS="true"
  BUILD_MAJOR_VERSION_SNAPSHOTS="true"
  PREMERGE_PROFILES="-Psnapshots,pre-merge"
fi
